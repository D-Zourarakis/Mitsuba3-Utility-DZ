...
def to_vec3(vec):
        return mi.ScalarPoint3f(*vec) if isinstance(vec, (list, tuple, np.ndarray)) else vec
...
class Camera:
    ...
    def load_sensor(self, angle=0.0):
        transform = mi.ScalarTransform4f().look_at(
            origin=to_vec3(self.rotate_camera_origin(angle)),
            target=to_vec3(self.camera_target),
            up=to_vec3(self.up_axis)
        )



        # Rotation around X-axis (30 degrees)
        rotation_x = mi.ScalarTransform4f().rotate(to_vec3([1, 0, 0]), float(config.x_rotation_degrees))

        # Rotation around Y-axis (45 degrees)
        rotation_y = mi.ScalarTransform4f().rotate(to_vec3([0, 1, 0]), float(config.y_rotation_degrees))

        # Rotation around Z-axis (60 degrees)
        rotation_z = mi.ScalarTransform4f().rotate(to_vec3([0, 0, 1]), float(config.z_rotation_degrees))
