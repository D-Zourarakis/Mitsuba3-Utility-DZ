...
def to_vec3(vec):
    import mitsuba as mi
    return mi.ScalarPoint3f(*vec) if isinstance(vec, list) else vec
...
class Scene:
    ...
    def get_lights_from_json(self):
        ...
        for light_obj in config.lights:
            if light_obj.emitter_type == "area":
                ...
                to_world = mi.ScalarTransform4f().translate(to_vec3(position)).scale(scale_vector) @ rotation
    ...
    def load_scene(self):
        ...
        for object in self.objects:
            rotation_x = mi.ScalarTransform4f().rotate(to_vec3([1, 0, 0]), float(object.x_rotation_degrees))
            rotation_y = mi.ScalarTransform4f().rotate(to_vec3([0, 1, 0]), float(object.y_rotation_degrees))
            rotation_z = mi.ScalarTransform4f().rotate(to_vec3([0, 0, 1]), float(object.z_rotation_degrees))
        ...
        bsdf_checkerboard = {
            ...
            "reflectance": {
                "type": "checkerboard",
                "to_uv": mi.ScalarTransform4f().scale(to_vec3(checkerboard_scale_vector)),
                "color0": {
                    "type": "rgb",
    ...
    def get_lights_position_info(self, light : cf.LightConfig):
        ...
        if(light.emitter_position == "top-center"):
            ...
            rotation = mi.ScalarTransform4f().rotate(to_vec3(light.emitter_rotation_axis), float(light.emitter_rotation_degrees))

        if(light.emitter_position == "bottom-center-back" or (light.emitter_shape == "rectangle" and isinstance(light.emitter_position,str))):
            position = np.array(self.center) - np.array(bottom_offset)* np.array(self.camera.up_axis) - np.array(distances) * np.array(self.camera.depth_axis)
            
            if list(self.camera.up_axis) == [1, 0, 0]:
                if list(self.camera.depth_axis) == [0, 1, 0]:
                    rotation = mi.ScalarTransform4f().rotate(to_vec3(self.camera.up_axis), -90)
                elif list(self.camera.depth_axis) == [0, -1, 0]:
                    rotation = mi.ScalarTransform4f().rotate(to_vec3(self.camera.up_axis), 90)
            elif list(self.camera.up_axis) == [-1, 0, 0]:
                if list(self.camera.depth_axis) == [0, 1, 0]:
                     rotation = mi.ScalarTransform4f().rotate(to_vec3(self.camera.up_axis), 90)
                elif list(self.camera.depth_axis) == [0, -1, 0]:
                    rotation = mi.ScalarTransform4f().rotate(to_vec3(self.camera.up_axis), -90)
            elif list(self.camera.up_axis) == [0, 1, 0]:
                if list(self.camera.depth_axis) == [1, 0, 0]:
                     rotation = mi.ScalarTransform4f().rotate(to_vec3(self.camera.up_axis), 90)
                elif list(self.camera.depth_axis) == [-1, 0, 0]:
                     rotation = mi.ScalarTransform4f().rotate(to_vec3(self.camera.up_axis), -90)
            elif list(self.camera.up_axis) == [0, -1, 0]:
                if list(self.camera.depth_axis) == [1, 0, 0]:
                    rotation = mi.ScalarTransform4f().rotate(to_vec3(self.camera.up_axis), -90)
                    ...
                    rotation = mi.ScalarTransform4f().rotate(to_vec3(self.camera.up_axis), 90)
            elif list(self.camera.up_axis) == [0, 0, 1]:
                rotation = mi.ScalarTransform4f().rotate(to_vec3(self.camera.side_axis), -90)
            
            if list(self.camera.depth_axis) == [0, 0, 1]:
                rotation = mi.ScalarTransform4f().rotate(to_vec3(self.camera.side_axis), 0)
            elif list(self.camera.depth_axis) == [0, 0, -1]:
                rotation = mi.ScalarTransform4f().rotate(to_vec3(self.camera.side_axis), 180)
        
        #No point of rotating a sphere object
        if light.emitter_shape == "sphere":
            rotation = mi.ScalarTransform4f().rotate(to_vec3([1, 0, 0]), 0)
